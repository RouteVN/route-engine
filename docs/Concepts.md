...
- Its delay is the persisted `autoForwardDelay` minimum plus length-derived reading time divided by `autoForwardSpeed`. A speed of `1` is normal, `2` is twice as fast, and `0.5` is half speed. Reading time starts at 60ms per weighted grapheme in the current line's resolved dialogue text. Han, Hiragana, Katakana, and Hangul graphemes have a weight of 3, whitespace has a weight of 0.5, and other graphemes have a weight of 1. The result is capped at 20 seconds unless the configured minimum is already higher.
